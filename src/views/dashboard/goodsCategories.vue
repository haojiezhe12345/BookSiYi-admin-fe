...
function updateItem(index: number) {
    ...
    //console.log(itemId)
    if (itemId == -1) {
        axios.post('/admin/category/add', null, { params: { name: listData.value[index].categoryName } })
            .then((response) => {
        ...
        axios.put('/admin/category/udpate', {
            id: itemId,
...
function deleteItem(index: number) {
    ...
    axios.delete('/admin/category/delete', { params: { categoryId: itemId } })
        .then((response) => {
            if (response.data.code == 1) {
                fetchData()
